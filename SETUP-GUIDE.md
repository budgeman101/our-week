...
- **Lindsay's shifts** — on her card, tap **+ Add shift**. It pre-fills her usual start for that day — **5:30 PM on Mon/Tue, 4:00 PM on Fri** — so a normal night is one tap. Adjust the start for a different night, and add an end time if you like (it's fine to run past midnight, e.g. 12:30 AM or 2:30 AM). Shifts sit on a specific day, so summer's shifting schedule is just adding and removing them. If a shift is the same every week, tick **Repeats weekly**. Tap any shift to change or delete it.
