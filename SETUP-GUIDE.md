...
- **The Kitchen tab** — tap it and the screen becomes the **full Our Kitchen app** (the complete original: Shopping, Pantry, Meals, Recipes, receipt scanning, staples, insights — everything). It's the same install, same address, just the kitchen half of the app; its **Week** tab (bottom-left) brings you back here. One thing to know:
...
- **Lindsay's shifts** — her regular nights are already in every week: **Mon & Tue 5:30 PM, Fri 4:00 PM**. You only touch them when summer changes things:
  ...
  - **Off just this week** — tap the shift → **Skip this week**. It greys out for that week only and comes back next week. Tap it again → **Bring back this week** to undo.
  - **Stopped for good** — tap the shift → **Delete every week**.
  - The **Energy** line fills itself in from whatever shifts are active that day.
- **Energy follows the shifts** — on any day that has a shift (or comes after a night shift), the **Energy** line fills itself in automatically — "works tonight — light", "off a night shift + works tonight — lightest", "day shift — moderate", and so on — and shows a small *· from shifts* tag. Change the shifts and the energy updates. On days with no shift, the Energy line stays a normal editable note like before.
...
- **The Projects tab** — track multi-step jobs (like the stairs), and it mostly runs itself:
  ...
  - **Auto-schedules around Lindsay** — tap **Auto-schedule** on a project to spread its unfinished steps across the coming days. It fills each day with **as much as can reasonably be done** — about 4 things a person per day, counting what's already there (appointments, that day's cleaning, existing to-dos) so busy days get fewer. **Lindsay's steps skip the day after she works** (her recovery day, when she's wiped) and she gets fewer on a night she works — it reads her real shifts, so it follows summer changes. Ben's and Both steps fill any day with room; a single project won't cram more than a couple of steps onto one day; and anything you've dated by hand is left alone.
- **Plan my week** — the button at the top of the **List tab** does all of the above across *everything* at once: it takes your unscheduled List to-dos **and** every project's undated steps and spreads them over the coming days, filling each to a reasonable ~4-a-day and working around Lindsay's shifts. It only touches unscheduled items, so it's safe to tap whenever new stuff piles up.
